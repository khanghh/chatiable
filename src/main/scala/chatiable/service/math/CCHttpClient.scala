...
import chatiable.service.math.CCHttpClient._

import scala.concurrent.ExecutionContext.Implicits.global
import de.heikoseeberger.akkahttpcirce.FailFastCirceSupport._

final class CCHttpClient(
)(implicit
  ...
  private[this] def constructRequest(
    ...
    HttpRequest(
      method = method,
      uri = Uri(path).resolvedAgainst(BaseUri).withQuery(
        Query(queries ++ Map(
          "p" -> "0",
          "reqid" -> "CVISmQkZ",
  ...
  private[this] def decodeResponse[B <: CCResponse](
  ...
  def requestWithoutToken[B <: CCResponse](
    method: HttpMethod,
    path: String,
  ...
  def request[A <: CCRequest, B <: CCResponse]( // scalastyle:ignore parameter.number
    method: HttpMethod,
    path: String,
    requestBody: A
  )(implicit
...
private object CCHttpClient {

  val BaseUri = Uri("https://coccoc.com/")
